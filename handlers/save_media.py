...
async def save_batch_media_in_channel(bot: Client, editable: Message, message_ids: list):
    try:
        ...
        share_link = f"https://telegram.me/{Config.BOT_USERNAME}?start=Shakthimaan_{str_to_b64(str(SaveMessage.id))}"
...
async def save_media_in_channel(bot: Client, editable: Message, message: Message):
    try:
        ...
        share_link = f"https://telegram.me/{Config.BOT_USERNAME}?start=Shakthimaan_{str_to_b64(file_er_id)}"
        await editable.edit(
            f"**{caption} - {file_size}\n\n{share_link}**",
            reply_markup=InlineKeyboardMarkup(
                [[InlineKeyboardButton("Open Link", url=share_link)],
